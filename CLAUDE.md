...
- Phase 7 Redis-Vector entity/relation parity (v0.16.0-phase7, v0.16.0 GA): `RedisVectorStore` now supports entities and relations (was chunks-only). Each type gets its own RediSearch index. `capabilities()` updated to `entities=true`, `relations=true`. Images remain unsupported on Redis (tracked for v0.16.1). Decision D (pgvector hybrid) deferred to post-GA. Test count: 466 base, 521 all features.
